...
impl Codegen {
    ...
    fn emit_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Texture2D => "hipTextureObject_t".into(),
            Type::Sampler => "hipTextureObject_t".into(),
            Type::Array { inner, .. } => {
            ...
            Item::FnDecl {
                ...
                match stage {
                    Some(ShaderStage::Kernel) => {
                        ...
                        self.push_indent();
                        self.push(&format!(
                            "uint32_t {} = hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x;\n",
                            idx_name
                        ));
                    ...
                    _ => {} // Vertex/Fragment not supported in ROCm
                }
            }
        }
    }
    ...
    fn generate(&mut self, program: &Program) -> String {
        self.push("#include <hip/hip_runtime.h>\n");
